...
class DialogCreateProperty(object):
    def setupUi(self, Dialog, select_window_setup):
        ...
        self.error_text.setObjectName("property_add_error_label")
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label.setText(_translate("Dialog", "Название нового свойства:"))
        self.confirmButton.setText(_translate("Dialog", "Подтвердить"))


    def on_confirm_button_release(self, event):
        name = self.lineEdit.text()
        if not StorageItem.does_property_exist(self.select_window_setup.category, name) and not (name.lower() == "id"):
            StorageItem.add_property_column(self.select_window_setup.category, name)
            self.select_window_setup.get_all_data_and_put_into_table()
        else:
            self.error_text.setText(f'Свойство "{name.capitalize()}" уже существует.')
