...
class DialogRenameCategory(object):
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label1.setText(_translate("Dialog", "Старое название категории:"))
        self.label2.setText(_translate("Dialog", "Новое название категории:"))
    ...
    def on_confirm_button_release(self, event):
        ...
        if Category.does_name_exist(old):
            if Category.does_name_exist(new):
                self.error_text.setText(f'Категория под названием "{new}" уже существует.')
            else:
                Category.rename(old, new)
                self.main_window_setup.get_categories_and_put_in_scroll()
        else:
            self.error_text.setText(f'Категории под названием "{old}" не существует.')
