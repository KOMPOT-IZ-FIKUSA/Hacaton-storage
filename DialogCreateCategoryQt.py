...
class DialogCreateCategory(object):
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label.setText(_translate("Dialog", "Название новой категории:"))
    ...
    def on_confirm_button_release(self, event):
        ...
        c = len(name.replace("\'", "").replace('\"', "")) == 0
        if c:
            self.error_text.setText('Неккоректное название')
        elif not Category.does_name_exist(name):
            new_category = Category(name)
            new_category.save()
            StorageItem.add_property_column(new_category, "Модель")
            StorageItem.add_property_column(new_category, "На складе")
            StorageItem.add_property_column(new_category, "Списано")
            self.main_window_setup.get_categories_and_put_in_scroll()
            self.error_text.setText('')
        else:
            self.error_text.setText(f'Категория "{name}" уже существует.')
